...
(2):loudflare API Token(至少要有 Cloudflare Tunnel:Read 权限)

(3):Tunnel ID(每条隧道唯一,在 Zero Trust 后台的 Networks → Tunnels 里能看到)

(4):Tunnel Name(仅用于面板展示,方便识别)

触发 GitHub Actions 需要:

-1.目标仓库 xxxxx/xxxxx

-2.workflow 文件名(如 xxxxx.yml)或 workflow ID

-3.分支 ref(如 main)

-4.一个有 workflow 权限的 GitHub Token
...
Expiration:设置有效期
Repository access:选 "Only select repositories",勾选你要触发工作流的那个仓库
